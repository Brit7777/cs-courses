...
typedef struct Customer_type * customer_ptr;
...
void linked_customer_manager();
void insert_customer(customer_ptr * ptr);
void delete_customer(customer_ptr * ptr);
void print_customer(customer_ptr   ptr);
void find_max_mean_salary(customer_ptr ptr);
// main()---------------------------------------------------------------
...
void main(void)
{
 ...
 while (ans!=0)
 {
  switch(ans)
  {
   ...
   case 9: linked_customer_manager();
	   break;
...
int show_menu()
{
  ...
  printf("8. maxheap\n");
  printf("9. linked customer manager\n");
  printf("0. Exit\n");
...
void linked_customer_manager()
{
	...
	printf("3. Find Max and Mean salary\n");
	...
	while (ans >=1 && ans <=5)
	{
		if (ans >=1 && ans <=5)
		{
			switch(ans)
			{
			case 1: insert_customer(&customer);
				break;
			case 2: delete_customer(&customer);
				break;
			case 3: find_max_mean_salary(customer);
				break;
			case 4: print_customer(customer);
				break;
			case 5: exit(1);
			}
		}
...
void insert_customer(customer_ptr *ptr)
{
		...
		temp = (customer_ptr) malloc(sizeof(struct Customer_type));
		
		// get a value for each member of customer struct object	
		// name
		printf("name[20 chars] = "); 
		scanf("%s", &name);		
		...
		temp->salary = salary;
		// [Q] birth year
		printf("birthyear[0000] = "); 
		scanf("%d", &year);		
		temp->birth.year = year;// [Q] birth month
		printf("birth month[0..00] = "); 
		scanf("%d", &month);
		temp->birth.month = month;
		// [Q] birth day
		printf("birth day[00] = "); 
		scanf("%d", &day);	
		temp->birth.day = day;
		// [Q] make temp's link null
		temp->link = NULL;
		rear = *ptr;   // ptr is the list pointer			

		if (rear)	// find the last node of the list
		{
			while(rear->link != NULL)
				rear = rear->link;
				rear->link = temp;
...
void delete_customer(customer_ptr *ptr)
{
	...
	temp = *ptr;    // the list pointer
	prev = NULL;	// previous node of the deleted node

	while (temp)                        // find the given name 
	{
		if (!strcmp(temp->name,name))   // if name is found
			break;
		
		prev = temp;//[Q] prev should remember the previous node before temp moves
		temp=temp->link;//[Q] temp moves to the next following link
	}

	if (temp==NULL)                    // if the name is not found
	{
		printf("Such node is not found\n");
		return;
	}

	if (prev==NULL)        // if prev node does not exist, that is, the first node is deleted
	{
		(*ptr) = temp->link;// [Q] update ptr pointer

	}
	else                  // previous node exists
	{
		prev->link = temp->link;// [Q] update prev link pointer 
	}
	free(temp);
...
void print_customer(customer_ptr ptr)
{
	customer_ptr temp = ptr;
	
	printf("\n	name   age   salary    birthday \n");
	printf("==================================== \n");
	

	for ( ; temp; temp = temp->link) // print each node's members of nodes using temp
	{	
		printf("%10s ", temp->name);	
		printf("%8d", temp->age);
		printf("  ");
		printf("%.1f", temp->salary);
		printf("%5d", temp->birth.year);
		printf("%8d", temp->birth.month);
		printf("%8d\n", temp->birth.day);
	}
...
{
	int num=0;
	float total=0, max_salary;
	
	customer_ptr temp = ptr;
	customer_ptr max_customer;

	if (ptr)	// if list is not empty
	{
		max_salary = temp->salary;// [Q] assume the first node's salary is the max salary 
		max_customer = temp; // [Q] assume the first node is the max customer
	}
	else		// empty list
		return;
	...
	printf("Max salary = %5.1f \n", max_salary);
	printf("Max salary person = %10s \n", max_customer->name);
	printf("Mean Salary = %5.1f \n\n", total/num);
}